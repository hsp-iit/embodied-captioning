...
def _kl_distance(inputs):
    ...
    inputs = inputs / inputs.sum(-1, keepdim=True)
    inputs = inputs.clamp(min=1e-8, max=1.0 - 1e-8)
    return -(inputs * inputs.log()).sum(-1)


@baseline_registry.register_env(name="SemanticDisagreement-kl")
class SemanticDisagreement(BaseEnv):
    ...
    def _update_pointcloud(self, observations, map_bounds):
        ...
        self.pcd.distance_f = self.distance_f
        t = time.time()
