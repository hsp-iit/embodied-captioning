...
class AgentPoseSense(Pose):
    ...
    def load(path):
        ...
        location_data = np.load(path, allow_pickle=True)['arr_0']
...
class DepthSense(VisualSense):
    ...
    @staticmethod
    def load(path):
        depth_image = np.load(path)['arr_0']
...
class BBSense(VisualSense):
    ...
    @staticmethod
    def _load_bbs(path):
        raw_prediction = np.load(path, allow_pickle=True)['arr_0']
        instances = raw_prediction.item()['instances']
        if len(instances) == 0:
            return instances
        # TODO: Is necessary to filter prediction?
        # mask = [
    ...
    def get_bbs_as_gt(self):
        ...
        target.captions = self.bbs.captions
