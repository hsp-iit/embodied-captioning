...
@baseline_registry.register_trainer(name="goalexplorationbaseline-v2")
class GoalExplorationTrajectoryBaseline(Baseline):
    def __init__(self, config, exp_base_dir, detectron_args, **kwargs):
        ...
        # cumulative rewards over agent trajectories
        self.trajectory_rewards = None
        print("GOAL EXPLORATION TRAJECTORY")
    ...
    def goto_next_subgoal(self, idx):
        if (self.got_new_plan[idx] or self.current_steps[idx] % 10 == 0) and len(
            ...
            print("add rewards:", self.trajectory_rewards)
    ...
    def train(self) -> None:
        ...
        while not self.is_done():
            ...
            if self.num_steps_done % self.config.ppo.num_global_steps == 0:
                ...
                # self.g_reward = torch.from_numpy(
                #         np.array(
                #             [
                #                 self.envs.call_at(idx, "get_last_reward")
                #                 for idx in range(self.envs.num_envs)
                #             ]
                #         )
                #     ) #reward to be used on next training step
