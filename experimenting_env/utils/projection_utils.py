...
def project_semantic_masks_to_3d(
    ...
    observation_pcd = SemanticPointCloud(episode, torch.Tensor(), torch.Tensor())
    ...
    if update_logits:
        ...
        observation_pcd.update_embeddings(instances, infos)
...
def _project_using_open3d(
    ...
    pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
        rgbd,
        o3d_intrinsics,  # , np.linalg.inv(pose)
    )
...
class SemanticPointCloud:
    def __init__(
        ...
        # object id infos is 500000
        self._init_pcd()
        self.thr = 0.05
        self._episode = episode
        self.scene = None
        self.object_id_to_logits: Dict = dict()
        self.object_id_to_embeddings: Dict = dict()
    ...
    #         for old_key in x.object_id_to_logits.keys(): self.object_id_to_logits[key] = x.object_id_to_logits[old_key] self.infos[self.infos[:, -1] == old_key, -1] = key
    #             key += 1
    ...
    def __add__(self, x: "SemanticPointCloud"):
        ...
        if hasattr(self, "object_ids") and len(self.object_ids):
            # Objects embeddings are always concatenated. Call preprocess() for solving inconsistencies
            key = max(self.object_ids) + 1
            for old_key in x.object_id_to_embeddings.keys():
                self.object_id_to_embeddings[key] = x.object_id_to_embeddings[old_key]
                self.object_id_to_logits[key] = x.object_id_to_logits[old_key]
                self.infos[self.infos[:, -1] == old_key, -1] = key
                key += 1
        elif len(x.object_ids):
            self.object_id_to_embeddings = x.object_id_to_embeddings
            self.object_id_to_logits = x.object_id_to_logits
        self._init_pcd()
    ...
    def update_logits(self, instances: Instances, infos):
        ...
        for i in range(len(instances)):
            ...
            self.object_id_to_logits[object_id] = (
                logits  # self.object_id_to_logits.get(
            )
            # object_id, []
            # ) + [logits]

    def update_embeddings(self, instances: Instances, infos):
        """Receive instances prediction from the model and update the embeddings for each object_id"""

        for i in range(len(instances)):
            embeddings = instances[i].embeddings
            info = infos[i]

            object_id = info["id_object"]
            self.object_id_to_embeddings[object_id] = embeddings.reshape((1, 384))
    ...
    def preprocess(self, lower_bound=None, upper_bound=None):
        ...
        consistency_solution = partial(
            ...
            object_id_to_embeddings=self.object_id_to_embeddings,
        )
        ...
        for k in objects_to_update:
            ...
            if len(voxels_to_use):
                ...
                acc_embeddings = (
                    torch.cat([self.results[i][2] for i in voxels_to_use])
                    .squeeze()
                    .unique(dim=0)
                )
                resolved_logits[k] = acc_logits
                resolved_embeddings[k] = acc_embeddings
        ...
        self.object_id_to_embeddings = resolved_embeddings
    ...
    def get_topdown_semantic(self, lower_bound, upper_bound, map_scale=0.025):
        ...
        if len(self) and self.points is not None:
            ...
            # Disagreement mat
            for k in self.object_id_to_embeddings.keys():
                if len(self.object_id_to_embeddings[k].shape) < 2:
                    self.object_id_to_embeddings[k] = self.object_id_to_embeddings[
                        k
                    ].unsqueeze(dim=0)            
                if self.object_id_to_embeddings[k].shape[-1] < 384:
                    self.object_id_to_embeddings[k] = F.pad(self.object_id_to_embeddings[k], (0, 1, 0, 0))
                
            cosine_distances = {
                k: self.distance_f(
                    torch.cat(
                        [
                            embedding.reshape((384, 1))
                            for embedding in self.object_id_to_embeddings[k]
                        ],
                        dim=1,
                    )
                )
            ...
            # for k in self.object_id_to_embeddings.keys():
            #     print(
            #         f"object {k} have {len(self.object_id_to_embeddings[k])} embeddings"
            #     )
