...
class SenseInfo:
    ...
    def get_path(self) -> str:
        return os.path.join(
            self.base_path,
            f"episode_{self.episode:06d}_step_{self.step:05d}_modality_{self.mod}_id_{self.camera_id}.npz",
        )
...
def save_obs(exp_path, episode_id, observations, timestamp):
    ...
    paths = []
    for camera_id, (modality, camera_obs) in enumerate(observations.items()):
...
def _save_data(exp_path, episode_id, modality, camera_id, timestamp, data):
    ...
    path = f"{exp_path}/episode_{episode_id:06d}_step_{timestamp:05d}_modality_{modality}_id_{camera_id}.npz"
